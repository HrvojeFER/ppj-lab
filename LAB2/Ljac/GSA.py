...
class Table:
    def __init__(self, dfa, state_dictionary):
        self.dfa = DFA.DFA(dfa)

        self.action = dict()
        self.new_state = dict()

        self.build_action(state_dictionary)

    def build_action(self, state_dictionary):
        for state in state_dictionary:
            for lr_item_combo in state_dictionary[state]:
                if lr_item_combo[0].endswith("*"):
                    if lr_item_combo[0].startswith("<%>"):
                        if state not in self.action:
                            self.action[state] = dict()

                        self.action[state]["#"] = "prihvati()"
                    else:
                        for next_sign in lr_item_combo[1]:
                            if state not in self.action:
                                self.action[state] = dict()

                            self.action[state][next_sign] = "reduciraj(" + str(lr_item_combo[0][:-2] + ")")
                else:
                    for next_sign in lr_item_combo[1]:
                        next_state = self.dfa.transition(state, next_sign)

                        if next_state is not None:
                            if state not in self.action:
                                self.action[state] = dict()

                            self.action[state][next_sign] = "pomakni(" + str(next_state) + ")"
...
# DFA ima DKa u sebi, a states ima riječnik DKA stanja u LR stavku, dok LR stavka je zapravo kominacija LR stavke i sljedi znakova.
dfa, states = EquivalentStates.EquivalentStates.build_dfa_with_dict(LRDefinition.LRDefinition(gsa))
# DFA states u sebi sadrži set stanja DFA, makar mislim da je to super glupo jer states.keys() ima isto...
dfa_states = set(states.keys())
