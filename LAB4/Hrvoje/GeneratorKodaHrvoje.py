...
class PrijevodnaJedinica:
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.VANJSKA_DEKL:
            return self.__content[0].generate(helper)

        if content_type is OtherType.PRIJEVODNA_JED_I_VANJSKA_DEKL:
            return self.__content[0].generate(helper) + \
                   self.__content[1].generate(helper)
...
class PrimarniIzraz:
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.IZRAZ_U_ZAGRADAMA:
            return self.__content[1].generate(helper)

        if content_type is OtherType.IDN:
            helper.idn = self.__content[0]

        if content_type is OtherType.BROJ:
            pass

        if content_type is OtherType.ZNAK:
            return Command.build_move(ord(self.__content[0].get_value()[1:-1]), "R0") + \
                   Command.build_push("R0")

        if content_type is OtherType.NIZ_ZNAKOVA:
            to_write = ""
            for char in self.__content[0].get_value()[1:-1]:
                to_write += Command.build_move(ord(char), "R0") + \
                            Command.build_push("R0")
...
class PostfiksIzraz:
    ...
    def generate(self, helper):
        ...
        self.__content[0].generate(helper)

        if content_type is OtherType.PRIMARNI_IZRAZ:
            if not helper.get_idn:
                return Command.build_load("R0", "VAR_" + helper.var_adr[helper.idn.get_value()]) + \
                       Command.build_push("R0")

        if content_type is OtherType.POSTFIKS_UGLATE_ZAGRADE:
            return self.__content[2].generate(helper) + \
                Command.build_pop("R0") + \
                Command.build_move(helper.var_dict[helper.idn.get_value()], "R1") + \
                Command.build_add("R0", "R1", "R0") + \
                Command.build_add("R0", "R7", "R0") + \
                Command.build_load_from_register("R0", "R0", 0) + \
                Command.build_push("R0")

        if content_type is OtherType.POSTFIKS_PRAZNE_ZAGRADE:
            return Command.build_push("R0") + \
                   Command.build_push("R1") + \
                   Command.build_push("R2") + \
                   Command.build_push("R3") + \
                   Command.build_push("R4") + \
                   Command.build_push("R5") + \
                   Command.build_call("F_" + helper.idn.get_value()) + \
                   Command.build_pop("R0") + \
                   Command.build_pop("R1") + \
                   Command.build_pop("R2") + \
                   Command.build_pop("R3") + \
                   Command.build_pop("R4") + \
                   Command.build_pop("R5") + \
                   Command.build_push("R6")
        ...
        if content_type is OtherType.POSTFIKS_PUNE_ZAGRADE:
            return Command.build_push("R0") + \
                   ...
                   Command.build_push("R4") + \
                   Command.build_push("R5") + \
                   self.__content[2].generate(helper) + \
                   Command.build_call("F_" + helper.idn.get_value()) + \
                   Command.build_pop("R0") + \
                   Command.build_pop("R1") + \
                   Command.build_pop("R2") + \
        ...
        if content_type is OtherType.POSTFIKS_INC:
            return Command.build_load_from_register("R0", "R7", helper.var_dict[helper.idn.get_value()]) + \
                   Command.build_push("R0") + \
                   Command.build_add("R0", 1, "R0") + \
                   Command.build_store_from_register("R0", "R7", helper.var_dict[helper.idn.get_value() + 4])

        if content_type is OtherType.POSTFIKS_DEC:
            return Command.build_load_from_register("R0", "R7", helper.var_dict[helper.idn.get_value()]) + \
                   Command.build_push("R0") + \
                   Command.build_sub("R0", 1, "R0") + \
                   Command.build_store_from_register("R0", "R7", helper.var_dict[helper.idn.get_value() + 4])
...
class UnarniOperator:
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.UNARNI_MINUS:
            return Command.build_pop("R0") + \
                   Command.build_move(0, "R1") + \
                   Command.build_sub("R1", "R0", "R0") + \
                   Command.build_push("R0")

        if content_type is OtherType.UNARNI_TILDA:
            return Command.build_pop("R0") + \
                   Command.build_move(0, "R1") + \
                   Command.build_sub("R1", 1, "R1") + \
                   Command.build_xor("R0", "R1", "R0") + \
                   Command.build_push("R0")

        if content_type is OtherType.UNARNI_NEG:
            return Command.build_pop("R0") + \
                   Command.build_cmp("R0", 0) + \
                   Command.build_jr_with_condition("Z", 8) + \
                   Command.build_move(0, "R0") + \
                   Command.build_jr(4) + \
                   Command.build_move(1, "R0") + \
                   Command.build_push("R0")
...
class CastIzraz:
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.VISESTRUKI_CAST:
            return self.__content[3].generate(helper)
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.GT_IZRAZ:
            return self.__content[0].generate(helper) + \
                   ...
                   Command.build_pop("R0") + \
                   Command.build_pop("R1") + \
                   Command.build_cmp("R0", "R1") + \
                   Command.build_jr_with_condition("SGT", 8) + \
                   Command.build_move("0", "R0") + \
                   Command.build_jr(4) + \
                   Command.build_move("1", "R0") + \
                   Command.build_push("R0")
        ...
        if content_type is OtherType.LT_IZRAZ:
            return self.__content[0].generate(helper) + \
                   ...
                   Command.build_pop("R1") + \
                   Command.build_cmp("R0", "R1") + \
                   Command.build_jr_with_condition("SLT", 8) + \
                   Command.build_move("0", "R0") + \
                   Command.build_jr(4) + \
                   Command.build_move("1", "R0") + \
                   Command.build_push("R0")
        ...
        if content_type is OtherType.GTE_IZRAZ:
            return self.__content[0].generate(helper) + \
                   ...
                   Command.build_pop("R1") + \
                   Command.build_cmp("R0", "R1") + \
                   Command.build_jr_with_condition("SGE", 8) + \
                   Command.build_move("0", "R0") + \
                   Command.build_jr(4) + \
                   Command.build_move("1", "R0") + \
                   Command.build_push("R0")
        ...
        if content_type is OtherType.LTE_IZRAZ:
            return self.__content[0].generate(helper) + \
                   ...
                   Command.build_pop("R1") + \
                   Command.build_cmp("R0", "R1") + \
                   Command.build_jr_with_condition("SLE", 8) + \
                   Command.build_move("0", "R0") + \
                   Command.build_jr(4) + \
                   Command.build_move("1", "R0") + \
                   Command.build_push("R0")
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.EQ_IZRAZ:
            return self.__content[0].generate(helper) + \
                   ...
                   Command.build_pop("R1") + \
                   Command.build_cmp("R0", "R1") + \
                   Command.build_jr_with_condition("EQ", 8) + \
                   Command.build_move("0", "R0") + \
                   Command.build_jr(4) + \
                   Command.build_move("1", "R0") + \
                   Command.build_push("R0")
        ...
        if content_type is OtherType.NEQ_IZRAZ:
            return self.__content[0].generate(helper) + \
                   ...
                   Command.build_pop("R1") + \
                   Command.build_cmp("R0", "R1") + \
                   Command.build_jr_with_condition("NE", 8) + \
                   Command.build_move("0", "R0") + \
                   Command.build_jr(4) + \
                   Command.build_move("1", "R0") + \
                   Command.build_push("R0")
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.OP_LOG_I_IZRAZ:
            return self.__content[0].generate(helper) + \
                   ...
                   Command.build_pop("R0") + \
                   Command.build_pop("R1") + \
                   Command.build_move("R2", "0") + \
                   Command.build_cmp("R0", "0") + \
                   Command.build_jr_with_condition("Z", 16) + \
                   Command.build_move("R2", "1") + \
                   Command.build_cmp("R1", "0") + \
                   Command.build_jr_with_condition("Z", 4) + \
                   Command.build_move("R2", "1") + \
                   Command.build_push("R2")
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.OP_LOG_ILI_IZRAZ:
            return self.__content[0].generate(helper) + \
                   ...
                   Command.build_move("R2", "1") + \
                   Command.build_cmp("R0", "0") + \
                   Command.build_jr_with_condition("NZ", 16) + \
                   Command.build_move("R2", "0") + \
                   Command.build_cmp("R1", "0") + \
                   Command.build_jr_with_condition("NZ", 4) + \
                   Command.build_move("R2", "0") + \
                   Command.build_push("R2")
...
class IzrazPridruzivanja:
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.OP_PRIDRUZI_IZRAZ:
            helper.get_idn = True
            self.__content[0].generate(helper)
            helper.get_idn = False

            return self.__content[2].generate(helper) + \
                   Command.build_pop("R0") + \
                   Command.build_store("R0", "VAR_" + helper.var_adr[helper.idn.get_value()])

    def __str__(self):
        return "<izraz_pridruzivanja>"


# todo kasnije visestruko pridruzivanje
class Izraz:
    def __init__(self, content, parent):
        self.__content = list()
        self.__parent = parent
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.IF:
            return self.__content[2].generate(helper) + \
                   Command.build_pop("R0") + \
                   Command.build_cmp("R0", 0) + \
                   Command.build_jp_with_condition("Z", "IF_END_" + str(helper.enter_if())) + \
                   self.__content[4].generate(helper) + \
                   "IF_END_" + str(helper.leave_if())
        ...
        if content_type is OtherType.IF_S_ELSEOM:
            return self.__content[2].generate(helper) + \
                   ...
                   Command.build_jp_with_condition("Z", "IF_END_" + str(helper.enter_if())) + \
                   self.__content[4].generate(helper) + \
                   Command.build_jp("ELSE_END_" + str(helper.current_if())) + \
                   "IF_END_" + str(helper.current_if()) + self.__content[6].generate(helper) + \
                   "ELSE_END_" + str(helper.leave_if())
...
class NaredbaPetlje:
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.WHILE:
            return "LOOP_START_" + str(helper.enter_loop()) + self.__content[2].generate(helper) + \
                   Command.build_pop("R0") + \
                   Command.build_cmp("R0", 0) + \
                   Command.build_jp_with_condition("Z", "LOOP_END_" + str(helper.current_loop())) + \
                   self.__content[4].generate(helper) + \
                   "LOOP_END_" + str(helper.leave_loop())

        if content_type is OtherType.FOR_BEZ_INDEKSIRANJA:
            return self.__content[2].generate(helper) + \
                   "LOOP_START_" + str(helper.enter_loop()) + self.__content[3].generate(helper) + \
                   Command.build_pop("R0") + \
                   Command.build_cmp("R0", 0) + \
                   Command.build_jp_with_condition("Z", "LOOP_END_" + str(helper.current_loop())) + \
                   self.__content[5].generate(helper) + \
                   "LOOP_END_" + str(helper.leave_loop())
        ...
        if content_type is OtherType.FOR_SA_INDEKSIRANJEM:
            return self.__content[2].generate(helper) + \
                   ...
                   Command.build_cmp("R0", 0) + \
                   Command.build_jp_with_condition("Z", "LOOP_END_" + str(helper.current_loop())) + \
                   self.__content[6].generate(helper) + \
                   self.__content[4].generate(helper) + \
                   "LOOP_END_" + str(helper.leave_loop())
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.CONTINUE_OR_BREAK:
            if self.__content[0].get_type() is KeywordEnumeration.KR_CONTINUE:
                return Command.build_jp("LOOP_START_" + str(helper.current_loop()))

            if self.__content[0].get_type() is KeywordEnumeration.KR_BREAK:
                return Command.build_jp("LOOP_END_" + str(helper.current_loop()))

        if content_type is OtherType.RETURN_BEZ_POV_VRIJEDNOSTI:
            return Command.build_push("R5") + \
                   Command.build_ret()

        if content_type is OtherType.RETURN_SA_POV_VRIJEDNOSTI:
            return Command.build_pop("R6") + \
                   Command.build_push("R5") + \
                   Command.build_ret()
...
class DefinicijaFunkcije:
    ...
    def get_content_type(self) -> OtherType or None:
        if len(self.__content) is 6:
            if isinstance(self.__content[0], ImeTipa) and \
                ...
                if isinstance(self.__content[3], Keyword) and self.__content[3]\
                        .get_type() is KeywordEnumeration.KR_VOID:
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.FUNKCIJA_BEZ_PARAMETARA:
            return "F_" + self.__content[1].get_value() + \
                   Command.build_pop("R5") + \
                   self.__content[5].generate(helper)
        ...
        if content_type is OtherType.FUNKCIJA_S_PARAMETRIMA:
            return "F_" + self.__content[1].get_value() + \
                   Command.build_pop("R5") + \
                   self.__content[3].generate(helper) + \
                   self.__content[5].generate(helper)
...
class DeklaracijaParametra:
    ...
    def generate(self, helper):
        ...
        if content_type is OtherType.DEKLARACIJA_OBICNOG_PARAMETRA:
            return ""

        if content_type is OtherType.DEKLARACIJA_PARAMETRA_POLJA:
            return ""
...
class GenerativeTree:
    ...
    def generate(self, helper):
        ...
        if self.__root is not None:
            to_write += self.__root.generate(helper)

        to_write += Command.build_helpers()
...
class GeneratorHelper:
    ...
    def add_var(self, name):
        ...
        self.var_adr[name] = self.var_num
        return self.var_num


some_tree = GenerativeTree.parse(sys.stdin.read())
some_tree.generate(GeneratorHelper())
